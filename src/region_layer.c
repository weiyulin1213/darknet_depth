...
void resize_region_layer(layer *l, int w, int h)
{
    ...
    l->outputs = h*w*l->n*(l->classes + l->coords + 2);
    l->inputs = l->outputs;
...
{
	// stride is one map size: l.w*l.h
...
{
	// box index at that cell
    int n =   location / (l.w*l.h);
	// decide which cell, cell index [0 ~ w*h-1]
...
void forward_region_layer(const layer l, network net)
{
    ...
    for (b = 0; b < l.batch; ++b){
        for(n = 0; n < l.n; ++n){
            ...
            activate_array(l.output + index, 2*l.w*l.h, LOGISTIC); // TODO? This is x,y map
            index = entry_index(l, b, n*l.w*l.h, l.coords); 
            if(!l.background) activate_array(l.output + index,   l.w*l.h, LOGISTIC); // this is confidence map
            index = entry_index(l, b, n*l.w*l.h, l.coords + 1); // TODO DONE
            if(!l.softmax && !l.softmax_tree) activate_array(l.output + index, l.classes*l.w*l.h, LOGISTIC); // This is class map
        }
    }
    if (l.softmax_tree){
        int i;
        ...
        int index = entry_index(l, 0, 0, l.coords + !l.background);// TODO DONE
        softmax_cpu(net.input + index, l.classes + l.background, l.batch*l.n, l.inputs/l.n - 1, l.w*l.h, 1, l.w*l.h, 1, l.output + index);
    }
#endif
    ...
    *(l.cost) = 0;
    for (b = 0; b < l.batch; ++b) {
        if(l.softmax_tree){
            int onlyclass = 0;
            for(t = 0; t < 30; ++t){
                box truth = float_to_box(net.truth + t*(l.coords + 2) + b*l.truths, 1); // *
                if(!truth.x) break;
                int class = net.truth[t*(l.coords + 2) + b*l.truths + l.coords]; // *
                float maxp = 0;
                int maxi = 0;
        ...
        for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w; ++i) {
				// n should be box index per cell
                for (n = 0; n < l.n; ++n) {
                    int box_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 0); // TODO
                    box pred = get_region_box(l.output, l.biases, n, box_index, i, j, l.w, l.h, l.w*l.h);
                    float best_iou = 0;
                    for(t = 0; t < 30; ++t){
                        box truth = float_to_box(net.truth + t*(l.coords + 2) + b*l.truths, 1); // TODO
                        if(!truth.x) break;
                        float iou = box_iou(pred, truth);
                        if (iou > best_iou) {
                            best_iou = iou;
                    ...
                    int obj_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, l.coords); // TODO  This is confidence
        ...
        for(t = 0; t < 30; ++t){
            box truth = float_to_box(net.truth + t*(l.coords + 2) + b*l.truths, 1); // TODO done
            ...
            //printf("index %d %d\n",i, j);
            for(n = 0; n < l.n; ++n){
                int box_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 0); // TODO done
                box pred = get_region_box(l.output, l.biases, n, box_index, i, j, l.w, l.h, l.w*l.h);
                if(l.bias_match){
			...
			// ----------------- box loss ---------------------
            int box_index = entry_index(l, b, best_n*l.w*l.h + j*l.w + i, 0); // TODO done
			// calculate box gradient
            float iou = delta_region_box(truth, l.output, l.biases, best_n, box_index, i, j, l.w, l.h, l.delta, l.coord_scale *  (2 - truth.w*truth.h), l.w*l.h);
            if(l.coords > 4){
                int mask_index = entry_index(l, b, best_n*l.w*l.h + j*l.w + i, 4); //  TODO no need to fix, not modified
                delta_region_mask(net.truth + t*(l.coords + 2) + b*l.truths + 6, l.output, l.coords - 4, mask_index, l.delta, l.w*l.h, l.mask_scale);
            }
            if(iou > .5) recall += 1;
            avg_iou += iou;
            ...
            int obj_index = entry_index(l, b, best_n*l.w*l.h + j*l.w + i, l.coords);  //TODO
			...
			int depth_index = entry_index(l, b, best_n*l.w*l.h + j*l.w + i, l.c-l.n-best_n*(l.coords+l.classes));
            ...
            int class_index = entry_index(l, b, best_n*l.w*l.h + j*l.w + i, l.coords + 1); // TODO done
            delta_region_class(l.output, l.delta, class_index, class, l.classes, l.softmax_tree, l.class_scale, l.w*l.h, &avg_cat, !l.softmax);
            ++count;
            ++class_count;
        }
...
void get_region_boxes(layer l, int w, int h, int netw, int neth, float thresh, float **probs, box *boxes, float **masks, int only_objectness, int *map, float tree_thresh, int relative) // TODO
{
    int i,j,n,z;
    ...
    for (i = 0; i < l.w*l.h; ++i){ // for every cell in predictions
        ...
        for(n = 0; n < l.n; ++n){ // for every box in one cell
            ...
            int obj_index  = entry_index(l, 0, n*l.w*l.h + i, l.coords); // TODO done
            ...
            int class_index = entry_index(l, 0, n*l.w*l.h + i, l.coords + !l.background); // TODO done: ...
            if(l.softmax_tree){
                ...
                float max = 0;
                for(j = 0; j < l.classes; ++j){
                    int class_index = entry_index(l, 0, n*l.w*l.h + i, l.coords + 1 + j); // TODO done: ... 
                    float prob = scale*predictions[class_index];
                    probs[index][j] = (prob > thresh) ? prob : 0;
			...
			int depth_index = entry_index(l, 0, n*l.w*l.h + i, (l.c-l.n)-n*(l.coords+l.classes));
...
void forward_region_layer_gpu(const layer l, network net)
{
    ...
    for (b = 0; b < l.batch; ++b){
        for(n = 0; n < l.n; ++n){
            int index = entry_index(l, b, n*l.w*l.h, 0); // TODO done
            activate_array_gpu(l.output_gpu + index, 2*l.w*l.h, LOGISTIC);
            if(l.coords > 4){
                index = entry_index(l, b, n*l.w*l.h, 4); // TODO no need to fix, not modified
                activate_array_gpu(l.output_gpu + index, (l.coords - 4)*l.w*l.h, LOGISTIC);
            }
            index = entry_index(l, b, n*l.w*l.h, l.coords); // TODO done
            if(!l.background) activate_array_gpu(l.output_gpu + index,   l.w*l.h, LOGISTIC); // confidence map
            index = entry_index(l, b, n*l.w*l.h, l.coords + 1); // TODO done
            if(!l.softmax && !l.softmax_tree) activate_array_gpu(l.output_gpu + index, l.classes*l.w*l.h, LOGISTIC); // class map
        }
    }
    if (l.softmax_tree){
        int index = entry_index(l, 0, 0, l.coords + 1); // TODO
        softmax_tree(net.input_gpu + index, l.w*l.h, l.batch*l.n, l.inputs/l.n, 1, l.output_gpu + index, *l.softmax_tree);
    } else if (l.softmax) {
        int index = entry_index(l, 0, 0, l.coords + !l.background); // TODO done
        //printf("%d\n", index);
        softmax_gpu(net.input_gpu + index, l.classes + l.background, l.batch*l.n, l.inputs/l.n - 1*l.w*l.h , l.w*l.h, 1, l.w*l.h, 1, l.output_gpu + index); // TODO
    }
